<#
.Synopsis
   Gets the CF Header for a rest calls
.DESCRIPTION
...
function Get-CFHeader {
    ...
    process {
        ...
        if ($null -eq $header) {
            $message = "header is not set in script variable. Call Get-CFToken first"
        ...
        if ($null -eq $token) {
            $message = "token is not set in script variable. Call Get-CFToken first"
            Write-Error -Message $message
            throw $message
        }
